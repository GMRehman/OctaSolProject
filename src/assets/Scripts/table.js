    ...
    if ( this.datatypes === undefined ) {
        ...
        this.datatypes = {
            ...
            "date": {
                ...
                "format": function (val) {
                    if (val == '') return;
                    var d = new Date(val.split(".")[2] / 1, (val.split(".")[1] / 1) - 1, val.split(".")[0] / 1);
...
Table.prototype.serialize = function(direction) {
    ...
    $(rows).each(function () {
        ...
        $(this).children("td").each(function () {
            ...
            var field = self._getfield(cid);
            var val = $(this).text() || "";
            /*****************Issue Fixed the edit item value was not serialed so fixed*************/
            if (val == "") {
        ...
        $(this).children("td.edit").each(function () {

            //alert($(this).children("input").text())
           // var cid = $(this).attr("id");
          //  var field = self._getfield(cid);
          //  var val = $(this).text() || "";
...
$(document).ready(function () {
    ...
    if (GetQueryStringParams("a") == undefined) {
        var paxdata = localStorage.getItem('pxdetail');

        // alert(paxdata);
        if (paxdata != null) {
            serverdata = JSON.parse(paxdata);
            // alert(serverdata);
        }
    }
    // Render the table
    for (i = 0; i < serverdata.length; i++) {
        t.data.push(serverdata[i]);
    }
    ...
    $("button#addrow").on("click", function () {
        t.data.push([t.data.length + 1, '', '', '', '', '']);
        t.render();
        savelocal();
    });
    // Serialize
    $("#cphContent_btnSave").on("click", function () {
    
        //$( t.id + ' tr td:first').click();
        var data = t.serialize();
        localStorage.removeItem('pxdetail');
        // alert(JSON.stringify(data));
        $('#cphContent_hid1').val(JSON.stringify(data));
       
    });
    $('#paxtable').on("keyup", function () {
        savelocal()
    });
    $('#paxtable').on("click", function () {
...
function savelocal() {
    if (GetQueryStringParams("a") == undefined) {
        var data = t.serialize();
        paxdata = JSON.stringify(data);
        localStorage.setItem('pxdetail', paxdata);
    }
}
